...
import { useFetch } from "@/components/ui/use-fetch"; // [!code ++] [!code focus]


interface Post { // [!code highlight]
  userId: number; // [!code highlight]
...
function PostComponent({ postId }) {
  ...
  const { data, loading, error, refetch } = useFetch<Post>(.  // [!code focus]
    `https://jsonplaceholder.typicode.com/posts/${postId}`.  // [!code focus]
  );                               // [!code focus]

  // [!code highlight] 1. Handle the loading state
  if (loading) {
    return <div>Loading post...</div>; // [!code highlight]
  } // [!code highlight]
